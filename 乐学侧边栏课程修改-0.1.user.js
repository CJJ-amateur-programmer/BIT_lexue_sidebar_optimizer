...
<button id="rewrite_sidebar" style="margin:1% 1% 1% 1%;">完成</button></div>`;
        ...
        popup.querySelector("#rewrite_sidebar").onclick=()=>{
            ...
            shown_courses=[];
            hidden_courses=[];
            var shown_lis=popup.querySelectorAll("#shown_courses>li"),
                hidden_lis=popup.querySelectorAll("#hidden_courses>li");
            for(var i=0;i<shown_lis.length;i++){
                shown_courses.push([shown_lis[i].dataset.id,shown_lis[i].innerText]);
            }
            for(var j=0;j<hidden_lis.length;j++){
    ...
    function reload_courses(){
        ...
        xhr.onreadystatechange=function(){
            if(xhr.readyState==4&&xhr.status==200){
                shown_courses=[];
                hidden_courses=[];
                var a=new DOMParser().parseFromString(xhr.responseText,'text/html').querySelectorAll("a[href*='lexue.bit.edu.cn/user/view.php']");
